	...
	def do_POST(self):
		...
		try:
			...
			if len(path_segments)>1:
				...
				# whereas a a POST request with path "/propagate..." won't be retransmitted to avoid infinite loops
...
if __name__ == '__main__':
	...
	if len(sys.argv) != 3: # 2 args, the script and the vessel name
		...
		for i in range(1, int(sys.argv[2])+1):
			vessel_list.append("10.1.0.%d" % i) # Add server itself, test in the propagation
